...
//
// LocalStore is a storage backend used as a tusd.DataStore in tusd.NewHandler.
...
// interface, which is implemented by LocalStore
package localstore
...
type LocalStore struct {
	// Relative or absolute path to store files in. LocalStore does not check
...
func (store LocalStore) UseIn(composer *tusd.StoreComposer) {
...
func (store LocalStore) NewUpload(info tusd.FileInfo) (id string, err error) {
	id = uid.Uid()
	info.ID = id
...
func (store LocalStore) WriteChunk(id string, offset int64, src io.Reader) (int64, error) {
...
func (store LocalStore) GetInfo(id string) (tusd.FileInfo, error) {
	info := tusd.FileInfo{}
...
func (store LocalStore) GetReader(id string) (io.Reader, error) {
	return os.Open(store.binPath(id))
}

func (store LocalStore) Terminate(id string) error {
...
func (store LocalStore) ConcatUploads(dest string, uploads []string) (err error) {
...
func (store LocalStore) LockUpload(id string) error {
...
func (store LocalStore) UnlockUpload(id string) error {
	lock, err := store.newLock(id)
	if err != nil {
		return err
	}
...
func (store LocalStore) newLock(id string) (lockfile.Lockfile, error) {
...
func (store LocalStore) binPath(id string) string {
	return filepath.Join(store.Path, id+".bin")
}

// infoPath returns the path to the .info file storing the file's info.
func (store LocalStore) infoPath(id string) string {
	return filepath.Join(store.Path, id+".info")
}

// writeInfo updates the entire information. Everything will be overwritten.
func (store LocalStore) writeInfo(id string, info tusd.FileInfo) error {
